...
public class BtcProcessor implements PageProcessor {
	private static final Logger logger = LoggerFactory
			.getLogger(BtcProcessor.class);
	...
	public void process(Page page) {
		...
		if (page != null) {
			...
			for (Map.Entry<String, Object> entry : jsonObj.entrySet()) {
				JSONObject itemJsonObj = JSON.parseObject(String.valueOf(entry
						.getValue()));
				if (itemJsonObj.containsKey("ticker")) {
					String mStr = itemJsonObj.getString("ticker");
					BtcModel model = JSON.parseObject(mStr, BtcModel.class);

					if (model != null) {
						logger.debug(model.toString());
						model.type = entry.getKey();// 设置类型
						if (CacheManager.getInstance().getCache(model.type) == null) {
							CacheManager.getInstance().putCache(model.type,
									"20170101010101");
						}
						logger.debug("low is "+ model.low + " 低于多少 "+(model.low + (model.low * 0.02)));;
